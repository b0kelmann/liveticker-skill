...
| 22 | **Multi-Event container, exactly one active**: state holds N events, each with `mode: setup \| live`. The agent reasons against the single `live` event; `setup` events accept imports/edits but no `/post`/`/signal`. *(NEW 2026-05-06)* | active | Tom's framing: "x Events anlegen können, der Agent koordiniert immer 1". Setup happens per-event so the user can prepare next week's conference while last week's wedding is still active, then activate. Implication: `STATE` becomes a container (`EventStore` over `dict[event_id → EventBundle]`); reasoning + signal endpoints route through `STATE.current()`. Activating an event demotes the previous live event back to `setup`. |
| 23 | **Auto-crawl on URL imports**: a single URL triggers a depth-2 BFS crawl (max ~8 pages). Outbound links are scored against a DE+EN keyword list (schedule/agenda/programm/speaker/...) — top-scoring links are followed automatically; negative-keyword links (impressum/datenschutz/sponsors/login/...) are skipped. Stays same-origin. *(NEW 2026-05-06)* | active | Most event sites split content across Hauptseite + /schedule + /speakers (often with German *or* English navigation). One-page fetch loses 80% of the substance. Auto-crawl matches Tom's mental model "ich werf ihm die Eventseite hin, er findet selbst Schedule und Speaker". Stays at one LLM call total — discovery is regex+heuristic, not LLM-based, so latency overhead is just the extra HTTP fetches (5–10s for ~5 pages). Pure-Python implementation (no BeautifulSoup dep). |
| 24 | **Multi-day Plan**: `PlanItem.day` (optional string), set by the LLM only when sources indicate a multi-day event. UI groups via collapsable `<details>` sections per day; single-day events render as a flat list. *(NEW 2026-05-06)* | active | Tom found that GOSIM Paris 2026 spans 3 days; the import filled 19 plan items in one flat list, mixing days. `day` is free-form ("Day 1" / ISO date / weekday) — the LLM is instructed to pick *one* format and stay consistent. Sort key is `(day, time)`. Known limitation: the Plan card auto-refreshes every 2s, so user-collapsed day-sections re-expand on refresh — currently mitigated by defaulting all days to open; a real tabs-with-state UI is a future polish. |
| 25 | **Event-aware Plan views**: PlanItem gets `track` (thematic schiene) + `tags` (free-form labels). EventBundle gets `view_modes: list[str]` — the LLM picks which views fit the event during import, from `["chronological", "by_day", "by_track", "by_where"]`. The Plan card renders a pill-bar of available views; selection is client-side (localStorage), so it survives HTMX 2s polling. *(NEW 2026-05-06)* | active | Tom: events differ in shape (multi-track conferences, festival with stages, single-track wedding) — UI must adapt per event, not be fixed. `track` is distinct from `where` because tracks (e.g. "Agentic AI Summit") can move between rooms. `tags` lets the LLM mark special items ("keynote", "workshop", "hackathon") for filter chips later. View-pill bar uses radio-style toggling with all views rendered in HTML and CSS-hidden — JS just toggles `.active` on click and restores from localStorage on `htmx:afterSwap`. |
